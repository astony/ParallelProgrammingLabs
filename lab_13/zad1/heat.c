...
int main(int argc, char *argv[]) {
  ...
  x_min = ((double)(p * n + 1 - id * n - i) * a + (double)(id * n + i) * b) /
          (double)(p * n + 1);

  i = n + 1;

  // wspolrzedna prawego punktu dla wezla id
  x_max = ((double)(p * n + 1 - id * n - i) * a + (double)(id * n + i) * b) /
          (double)(p * n + 1);
...
void heat_part(int n, int p, int id, double x_min, double x_max) {
  ...
  for (i = 0; i <= n + 1; i++) {
    x[i] = ((double)(i)*x_max + (double)(n + 1 - i) * x_min) / (double)(n + 1);
  }
  ...
  for (j = 1; j <= j_max; j++) {
    ...
    for (i = 1; i <= n; i++) {
      h_new[i] =
          h[i] +
          t_del * (k * (h[i - 1] - 2.0 * h[i] + h[i + 1]) / x_del / x_del +
                   2.0 * sin(x[i] * t));
    }

    // nowa chwila czasu
    t = ((double)(j - j_min) * t_max + (double)(j_max - j) * t_min) /
        (double)(j_max - j_min);
